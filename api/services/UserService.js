(function() {
    ...
    /**
     *  This is a public methods to get a user by userId and populates the address
     ...
     *  @param      cb is a callback
     */
    module.exports.getDataService = function(userId, cb) {
        sails.models['user'].findOne({ where: { id: userId } }).exec(function(err, results){
            if(err) {
                sails.log.error("getDataService");
                return cb(500, err);
            }
            return cb(err, results);
        });
    ...
    /**
     *  This is a public methods to get a user by userId and populates the merchant
     *  @param      userId is the id of the user to find
     *  @param      cb is a callback
     */
    module.exports.getUserMerchantService = function(userId, cb) {
        sails.models['user'].findOne({ where: { id: userId } }).populate('userMerchant').exec(function(err, results){
            if(err) {
                sails.log.error("getUserMerchantService, can't get user by id");
                return cb(500, err);
            }
            return cb(err, results);
        });
